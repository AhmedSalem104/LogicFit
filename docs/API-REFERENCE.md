...
## الكتالوج الكامل القابل للتدقيق

للحصول على **كل** endpoints بالتفصيل—طريقة HTTP، المسار، صلاحية الدخول، مدخلات
body/query/form، والاستجابة المعلنة—استخدم
[كتالوج API الكامل](API-ENDPOINT-CATALOG.md). الملف مولّد مباشرة من الـControllers
بواسطة `Scripts/Export-ApiEndpointCatalog.ps1`، وهو المرجع التفصيلي الذي يراجع مع
أي تعديل في العقد. يبقى هذا الملف مرجعًا مختصرًا للسياسات والتدفقات الشائعة.
