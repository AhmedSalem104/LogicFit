...
public class UpdateSubscriptionPlanCommandValidator : AbstractValidator<UpdateSubscriptionPlanCommand>
{
    ...
    public UpdateSubscriptionPlanCommandValidator(IApplicationDbContext context, ITenantService tenantService)
    {
        _context = context;
    ...
    private async Task<bool> BeUniqueName(UpdateSubscriptionPlanCommand command, string name, CancellationToken cancellationToken)
    {
        var tenantId = _tenantService.GetCurrentTenantId();
        return !await _context.SubscriptionPlans
            .AnyAsync(p => p.TenantId == tenantId && p.Name == name && p.Id != command.Id, cancellationToken);
    }
}
